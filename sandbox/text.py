...
# writes to file from string
def reader(file):
    with open(file, 'r') as f:
        return f.read()

# finds line with capital letter in string and appends it to a list
def find_line_cap(string):
    lines = string.split('\n')
    cap_lines_lst = []
    hash_lines_lst = []
    for line in lines:
        if len(line) > 2:
            if line[0].isupper():
                cap_lines_lst.append(line)
            elif line[0] == '#':
                hash_lines_lst.append(line)
    
    # merges two lists
    dic = dict(zip(cap_lines_lst, hash_lines_lst))
    return dic


# finds line with '#' in string and appends it to a list
def find_line(string):
    ...
    for line in lines:
        ...
        if obj['title']:
            lines_lst.append(obj)
        
    return lines_lst

# create a list of a string
def create_list(string):
    string = string.split(' ')
    return string 

# remove `#` from every string in a list
def remove_hash(lst):
    new_lst = []
    for string in lst:
        ...
        string = string.replace('стайлинг', '')
        string = string.replace('клип', ' клип')
        string = string.replace('костюма', ' костюма')
        new_lst.append(string)
    return new_lst

# takes dictionary and creates new dictionary with keys names 'title' and value names 'tags'
def create_dic_list(dic):
    dic_list = []
    for key, value in dic.items():
        new_dic = {}
        tag_names = create_list(value)
        new_dic['title'] = key
        new_dic['tagNames'] = tag_names
        new_dic['tags'] = remove_hash(tag_names)
        dic_list.append(new_dic)
    return dic_list

# creates a json file and saves text to it
def writer(file, text):
    with open(file, 'w') as f:
        f.write(text)

# compares two string for similarity and returns a number
...
lst = read_json("spisok.json")
new = []
for i in range(len(lst)):
    # print("Number: ", i, lst[i]['title'], "Item: ", items[i]['title'])
	items[i]['title'] = lst[i]['title']
	items[i]['tagNames'] = lst[i]['tagNames']
	items[i]['tags'] = lst[i]['tags']
	new.append(items[i])

writer('new.json', json.dumps(new, ensure_ascii=False))

# file = reader('spisok.txt')
# print(find_line_cap(file))
# writer('spisok2.json', json.dumps(create_dic_list(find_line_cap(file)), ensure_ascii=False))
